# Prompt-modelo para subagente — PR do Bitbucket

Use este prompt ao disparar cada subagente background de PR do Bitbucket. Substitua os campos `<...>`.

---

Code review de um Pull Request do **Bitbucket**.

**PR alvo:** workspace=`<workspace>`, repo_slug=`<repo_slug>`, pull_request_id=`<id>`.
...
4. **Verificação adversarial (F4):** tente **refutar** cada hipótese ("por que NÃO é bug?"). O MCP do Bitbucket não expõe leitura de arquivo arbitrário, então para callers/contratos fora do diff **spawne sub-subagentes explorer focados** (perguntas específicas) — se não houver ferramenta de subagente, baseie-se no que o diff e o contexto da task revelam, e o que não der pra confirmar vira `partial`. Aplique os FP traps da doutrina. Veredictos: **confirmed** → blocker (voto-duplo: 2ª verificação confirma o mesmo `arquivo:Llinha`, senão rebaixa) ou warning; **partial** (código inalcançável) → warning + "confirmar:"; **refuted** → descartado; **question** (intenção do autor / regra de negócio) → guardado, fora do retorno final.
