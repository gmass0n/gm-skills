...
2. Explore every target repo before interviewing; treat prompt claims as hypotheses. Record runtime/package manager, start command and bind host/port, environment source/precedence/schema, mock-versus-real provider/base URL/prefix, auth flow, and existing test. If platform-specific startup hazards may apply, read [stack-discovery.md](references/stack-discovery.md) before writing the recipe; unknowns become questions, never assumptions.
