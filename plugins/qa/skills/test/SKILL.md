...
- Keep frontend work strictly sequential in one fresh browser profile, and mobile work strictly sequential on one booted Maestro device/simulator. Run backend work concurrently only when the plan marks it independent.
...
- Track every server, tunnel, browser profile, Metro process, launched app/device, and synthetic-data handle created by this execution. On every exit path, clean up only tracked resources, leave externally owned/shared/in-app browser or device processes untouched, and report the cleanup state.

## Workflow

1. Stand up the stack from the plan, verify the stated ports remain free, then confirm every service booted. For mobile scenarios, also boot the device/simulator, start Metro, and install/launch the built app on the recorded variant before testing. Before any real-data mutation, execute the plan's non-production host/profile/account preflight; block the scenario when it cannot prove the recorded target.
2. Run the mandatory smoke-check. Produce a failed verdict immediately if it does not pass.
3. Execute scenarios in plan order and concurrency class. Before frontend QA, terminate only orphan browser processes or profile locks proven to be from prior QA runs; never terminate a shared or in-app browser, and take no action when ownership is uncertain. Then start with a clean browser profile, collect required UI/network/log evidence, and close it at the end. Before mobile QA, reset the app to a known state on the booted device (reinstall or clear app data), drive it through the Maestro MCP, and capture screenshots plus Metro/device log lines as evidence.
