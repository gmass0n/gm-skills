...
**Peça um digest ENXUTO, com teto.** O briefing do `Explore` pede *só* o que distingue as hipóteses — a função suspeita + a **lista completa de callers (arquivo:linha)** + o trecho load-bearing —, **não** um tour da camada. Um digest que volta com 150+ KB é exploração exaustiva no lugar errado: na caça a bug o `Explore` confirma *onde a hipótese vive*, não documenta a arquitetura (isso é `sdd:codebase`). Alvo: cada digest cabe em poucos KB. Se o sintoma é um fluxo, peça **a cadeia de chamadas** (quem chama quem, do evento ao persist), não o corpo de cada função. A lista de callers é obrigatória e completa (é o que garante o fix na raiz); só o *corpo* das funções é que encolhe.

**Persista já:** grave as hipóteses no report com status `❓ não testada`. Se a sessão morrer, elas sobrevivem.

**Bug pontual vs. bug de cadeia — escolha o modo de instrumentação ANTES de hipotetizar fundo.** Se o sintoma é um valor/branch num ponto (tipo a/b) → poucos senders cirúrgicos nos pontos que separam as hipóteses (o default da F3). Mas se o bug é um **fluxo linear silencioso** (tipo c: `evento → handler → guard → throttle → fetch → persist`) onde *nada grita* e a dúvida real é "**em que elo o fluxo morre?**", não itere hipótese-a-hipótese sobre cada estágio — **instrumente a cadeia INTEIRA de uma vez, 1 sender por elo**, cada um marcando seu `stage` (schema em `references/runtime-capture.md`). Uma única reprodução revela o primeiro elo que não emite, e isso **mata várias hipóteses de uma vez** em vez de uma por rodada. Caso real: 5 hipóteses sobre o contrato de dados (lista→save→POST→response) custaram uma rodada inteira de instrumentação que só provou "contrato OK" sem achar a causa; instrumentar `event-fired → handler → http-sent → http-acked` numa passada teria mostrado `event-fired:0` no primeiro elo (o evento nunca dispara) e pulado direto à raiz. **Cadeia inteira ≠ spam:** é 1 sender por estágio nomeado (a cadeia tem 4–6 elos), não dezenas no mesmo ponto.
