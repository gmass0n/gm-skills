---
name: debug
description: "Hunt the root cause of a bug and apply a surgical fix — off the forward SDD track, triggerable at any time. Use whenever something BROKE: console error, stack trace, exception, on-screen/UI error, failing request, red test, wrong flow, wrong value/state, unexpected behavior, regression — or when the user says \"it's broken\", \"it's throwing an error\", \"debug this\", \"why doesn't this work\", \"fix this bug\", \"investigate this problem\". Generates SEVERAL root-cause hypotheses BEFORE any fix, instruments the code with hypothesis-driven logs sending evidence to a local debug server, collects real runtime (server + Playwright MCP, or the human reproducing) instead of guessing, applies the smallest fix at the root (grep the callers, not the symptom), verifies with a regression test, and CLEANS UP all instrumentation. Records a grounded lesson into docs/codebase/lessons/ when the cause was shared. Do NOT use to build a new feature or a broad refactor — that's sdd:spec / sdd:plan / sdd:implement."
---
