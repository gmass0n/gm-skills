---
name: review
description: "Enterprise-grade LOCAL code review of ANY target — the pre-flight before `git push`, when there is no remote PR yet. Use when the user says \"review my changes\", \"revisa antes do PR\", \"review this file/module\", \"review src/modules/orders\", \"audita o repo\", \"revisa esse trecho\", \"check this before I push\", or invokes `/sdd:review`. Reviews FIVE target kinds, all resolved by `scripts/review.js resolve`: a diff (default — the branch vs its GitFlow merge-base, or `--staged`/`--working`/`A..B`), a file/dir/glob (the whole content), a commit/range/tag, the whole repo (`--all`, with announced sampling), or a pasted snippet (`--paste`). Dispatches blind parallel reasoning lenses (correctness, security, performance, architecture/DRY-reimplementation, spec-alignment) — each fed ONLY its path slice, each returning findings as questions/hypotheses, never verdicts — then a per-file adversarial verification batch that tries to FALSIFY each one against the real `file:Lline` before it counts; executable gates (test/typecheck/lint) and deterministic invariant checks run in PARALLEL; a risk-gated mutation sensor proves the tests discriminate; a loop re-runs ONLY when a NEW confirmed blocker appears (cap 2). Codebase-aware via docs/codebase/context.md (degrades to ungrounded, never refuses); bias-to-approve (only a confirmed blocker rejects); writes the verdict to docs/reviews/<slug>/review.md with a machine-readable trailer. It ROUTES, it does not fix (runtime bug → sdd:debug; coverage/requirement gap → sdd:plan/sdd:spec). Do NOT use for a remote PR with a link (that's prs-review, via MCP) or intra-task during a build (that's sdd:implement's Post-Gate Review)."
---
