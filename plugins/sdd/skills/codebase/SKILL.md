...
- **Test-suite signal (drives `conventions/testing.md`):** detect the test runner (manifest dep / `test`/`spec` script) and the test-file convention actually used (`*.test.*`, `*.spec.*`, `*_test.*`, `test_*.*`, a `tests/`/`__tests__/`/`spec/` dir). If any exists, `conventions/testing.md` is in the plan — not optional.
