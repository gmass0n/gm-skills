...
```
docs/codebase/
├── context.md           # single agent-optimized router: enforced invariants + stack + navigation + per-task loading + catalog
├── overview.md          # onboarding narrative: end-to-end flow + principles (reading, not routing)
├── architecture/        # macro architecture — one doc per architectural decision/style
├── layers/              # per-layer rules — one doc per real project layer
├── patterns/            # tactical and cross-cutting patterns — one doc per real pattern
├── integrations/        # external libs/services wired into the runtime — one doc per integration
├── conventions/         # conventions and enforcement — one doc per convention family
...
1. **Enforced invariants** — the project's non-negotiable rules, **each paired with the mechanism that imposes it** (the lint rule, the hook, the type, the interceptor — with its real path). Example: `domain does not import framework → eslint-plugin-boundaries (eslint.config.mjs)`; `coverage ≥ 80% → lefthook pre-push`. These are **generated by reading the real enforcers**, not authored by hand. A rule with no enforcer is an aspiration — list it under a separate "conventions (no enforcement)" note or drop it. This section is what lets a later phase mechanically answer "does this plan violate a project invariant?".
2. **Stack summary** — concern → tech → canonical doc link (one line each).
3. **Navigation by intent** — "if you need X → start at `dir/`" (serves human and agent).
4. **Per-task loading** — explicit pointers: "task of type X → load these docs". This is the bridge that makes selective loading work without the consumer guessing. E.g. `new HTTP feature → layers/presentation-layer.md + patterns/mapper-pattern.md + conventions/testing.md`.
5. **Catalog** — one bullet per real doc in the six dirs (relative link + one-line hook). The former README's job.

`context.md` does **not** hold the end-to-end trace — that is narrative reading and lives in `overview.md` (which is marked "onboarding, do not load for routine tasks").

`concerns/` is the one **evaluative** directory — it documents what is *wrong or fragile*, not how the code works. It is governed by a stricter evidence bar (every concern needs a `path:line` anchor, a confirmed `TODO`/`FIXME`, a proven test gap…) and its docs do **not** open with `Golden rule`. An honestly short `concerns/` beats an inflated one — when in doubt, omit. See `templates/docs/concern.template.md`.
...
2. **Carries the token-frugal loading rule**: "open the *one* doc your task needs; do not read the whole `docs/codebase/` tree by default." This is the same selective-loading discipline `context.md`'s "Per-task loading" encodes — the routers must echo it or agents over-read.
...
All generated docs are **English** (headers, prose, and explanations). This skill has no human interview, so the output language is unconditional. Code identifiers, file paths, and library names stay verbatim.
...
generated: 2026-06-22   # date of this generation (passed by you to the agent — do not compute it in the agent)
sources:                # the globs/paths THIS doc describes — the honest staleness basis
  - src/modules/**/domain/
  - src/shared/domain/
---
```
...
Every document under the five **descriptive** directories (`architecture/`, `layers/`, `patterns/`, `integrations/`, `conventions/`) follows the same archetype below. `concerns/` is **evaluative** and follows a different shape — findings anchored to real paths, no `Golden rule` — detailed in its own archetype. This consistency is what makes the map navigable: a reader who has seen one pattern doc knows exactly where to look in any other.

```markdown
# <Concept name>

## Golden rule              # the central invariant, in 1–3 sentences. A reader who only reads this gets it 80% right.

## <where it is / how it works>  # REAL code from the repo, with `// path/to/file` as a comment
                                 # show the thing, don't describe it

## <enforcement / mechanical rule>  # if a lint/CI/type ENFORCES the rule, cite the real snippet
                                    # (omit if the repo enforces nothing)

## Anti-patterns            # list with ❌ — what looks right but violates the rule
```

Reference archetypes live in `templates/docs/`. Each agent reads the archetype matching its directory:

| Directory | Archetype | Size target | Captures |
|---|---|---|---|
| `architecture/` | `templates/docs/architecture.template.md` | ~150–350 lines | Macro styles/decisions: dependency rule, ports & adapters, CQRS, bounded contexts, monorepo layout |
| `layers/` | `templates/docs/layer.template.md` | ~120–250 lines | One real project layer (domain, application, infra, presentation — or whatever the repo uses) |
| `patterns/` | `templates/docs/pattern.template.md` | ~80–200 lines | One concrete tactical pattern (entity, value-object, mapper, facade, DI, error-handling…) |
| `integrations/` | `templates/docs/integration.template.md` | ~15–60 lines | One external lib/service: what it is, where it is wired, rules |
| `conventions/` | `templates/docs/convention.template.md` | ~80–200 lines | Naming, testing, quality gates, lint boundaries |
| `concerns/` | `templates/docs/concern.template.md` | ~20–120 lines | Tech debt, risks, fragile areas — **evaluative**, no `Golden rule`, findings anchored at `path:line` with severity |

`overview.md` follows `templates/overview.template.md` (~120–250 lines). `context.md` follows `templates/context.template.md` — it is assembled **last**, from the final list of files that actually exist (you need the real doc set to write the catalog and the per-task loading pointers).
...
- **Enforcer inventory (drives `context.md` › Enforced invariants):** find the *mechanisms* that make rules binding, not the rules in prose. Read the lint boundary config (`eslint-plugin-boundaries` rules, import rules), coverage thresholds and git hooks (`lefthook.yml`, `husky`, `.git/hooks`), global interceptors/guards/pipes that shape every response, and type-level contracts that pin invariants. Each becomes one "rule → mechanism (path)" line. A rule you can't tie to a mechanism is an aspiration — keep it out of "enforced".
...
- **`conventions/`** — naming, testing, quality gates, and any *mechanical* enforcement (lint boundary rules, coverage thresholds, commit hooks). The enforcement docs are the highest-value ones — they turn "we try to" into "the build fails if". **Testing gets its own dedicated `conventions/testing.md` whenever the repo has ANY test suite** (a test runner in the manifest, a `test`/`spec` script, or real `*.test.*`/`*.spec.*`/`*_test.*`/`test_*.*` files). It is not optional and is not allowed to be a thin paragraph folded inside a quality-gates doc — it covers: the test stack (runner, assertion/DOM libs, e2e tool), how tests are organized and named, the commands to run them, coverage config/thresholds if any, and the TDD/policy stance if the repo states one. Keep a separate quality-gates doc for lint/format/typecheck/hooks; testing is its own concern. Only omit `testing.md` if the repo genuinely has zero tests — and then say so in the report.
    **`testing.md` must state these three things explicitly** (they are the rules humans keep forgetting, so the map's whole point is to make them binding):
    ...
    3. **TDD stance** — if the repo declares test-first, state it as policy here, and list it under `context.md` › Enforced invariants if a hook/CI gate makes it binding (otherwise under "conventions (no enforcement)").
    The `## Anti-patterns` of `testing.md` names the literal mistakes these rules prevent — `❌ mapper/dto/controller without spec`, `❌ spec outside the tests/ of its own sub-layer` — so a reader who skims only that section still avoids them.
  - **CI/CD + container build:** when `Dockerfile`, `bitbucket-pipelines.yml` (or `.github/workflows/`), `.dockerignore` exist → plan `conventions/dockerfile.md` and `conventions/<ci-tool>-pipelines.md` (e.g. `bitbucket-pipelines.md`). The "build/CI/lint config presence … enforcement signal" already exists; these generate **dedicated** convention docs, never a new dir.
- **`concerns/`** — one doc per *risk area* that has **real, anchored** findings. Sweep `TODO`/`FIXME`/`HACK` markers, business-rule files missing co-located tests, **any artifact the testing convention requires a spec for (dto/mapper/controller/handler) that has none — a missing spec is an anchored, real concern, not a nitpick**, `@ts-ignore`/`eslint-disable`/`any` at sensitive points, security smells (hardcoded secret, unvalidated entrypoint input, weak crypto), perf smells (N+1, I/O in loops, unpaginated growth), stale deps. Group findings into files like `security-gaps.md`, `perf-hotspots.md`, `fragile-areas.md`, `tech-debt.md`. **Stricter evidence bar than every other directory** — a concern without a `path:line` anchor, a confirmed marker, or a proven gap is speculation; cut it. If a risk area has no anchored finding, do not create its file. If *nothing* is found anywhere, `concerns/` gets a single `tech-debt.md` noting "no relevant findings" — do not pad it.
  - **Machine-parseable contract (so `sdd:plan` can ingest scoped remediation):** each finding carries a stable `id`, a `severity` enum, and an anchor **with a line number**. This is non-negotiable for concerns because a later phase filters them by file path. The shape lives in `concern.template.md`:
    ```
    - id: CONCERN-007
      severity: high              # enum: high | medium | low
      anchor: src/modules/notification/seru-notification.adapter.ts:42
      description: upstream stream timeout not handled
    ```
    A bare `Anchor:` without `:line` or a severity written in prose is not parseable — give the line and the enum.
...
Each agent returns finished English markdown for its one file. **You** write the files after agents return — subagents cannot persist host-filesystem writes.
...
1. **Enforced invariants** — turn the Step 1 enforcer inventory into "rule → mechanism (path)" lines. Only rules tied to a real mechanism (lint/hook/type/interceptor). Rules without enforcement go in a short "conventions (no enforcement)" note, not here. This section is the contract `sdd:plan`'s `/analyze` checks a plan against — get the mechanism path right.
2. **Stack summary** — concern → tech → canonical doc link, one line each.
3. **Navigation by intent** — reader intent → entry point (`dir/`).
4. **Per-task loading** — "task of type X → load these docs", using the real doc set. This is what lets a consumer load only what a task needs.
5. **Catalog** — every generated file as a relative link with a one-line hook. Group `patterns/` by sub-theme (Domain / Application / Boundaries / Presentation / Cross-cutting) when there are enough to warrant it; otherwise a flat list.
...
4. Dispatch agents only for affected docs. Each diff-agent reads the existing doc + the changed files and returns it with a `## Changelog` line appended (`- YYYY-MM-DD: <what changed>`) and its `generated:` frontmatter date refreshed. Pass the date in — do not let the agent compute it.
5. If the set of files changed (a doc added or removed), **regenerate `context.md`** (Step 3). Even if the file set is unchanged, refresh `context.md`'s "Enforced invariants" if an enforcer config (lint boundaries, hooks, coverage threshold, a global interceptor) was among the changed paths — a changed enforcer means the contract `sdd:plan` checks against changed. Otherwise leave `context.md` byte-identical.
...
| lint/boundary/test config, coverage threshold, hooks | `conventions/*` **and** `context.md` › Enforced invariants (the mechanism changed) |
...
- Spot-check that each `concerns/` finding carries a real `anchor: path:line`, a `severity` enum, and a stable `id`. A concern that isn't machine-parseable defeats the `sdd:plan` ingestion — cut it or fix it.
...
- **Coverage gate (full mode).** Before declaring done, cross-check the doc plan against the recon facts so nothing real went unmapped:
  ...
  - **CI/CD & container:** if there is a `Dockerfile` or a pipeline file (`bitbucket-pipelines.yml`, `.github/workflows/`), the corresponding convention docs (`conventions/dockerfile.md`, `conventions/<ci>-pipelines.md`) must exist — parity with the testing rule.
  ...
  - **Enforced invariants parity:** every enforcer found in Step 1 (lint boundary rule, coverage threshold, hook, global interceptor) appears as a line in `context.md`. A real enforcer with no line is a gap — the consuming phase would miss a project rule.
...
- **Every claim cites a real path.** No path = not actionable = cut it.
- **Golden rule first** (in the five descriptive dirs). Every doc in `architecture/`, `layers/`, `patterns/`, `integrations/`, `conventions/` opens with the invariant. If you can't write it in three sentences, you don't understand the concept yet — read more code. `concerns/` is the exception: it is evaluative, opens at the findings, with no golden rule.
- **Show real code, not pseudocode.** Excerpts come from the repo, with the path in the comment. The example proves the rule.
- **Cite the enforcement when it exists.** "Enforced by X" is worth ten times "the convention is to". Look for lint rules, type checks, CI gates, hooks.
- **Observed, not ideal.** Document what the code does, including inconsistencies. Do not prescribe what it *should* be.
- **Omit the empty.** No external integrations → a short note, not invented entries. No real anti-patterns → cut the section.
- **Sample, don't exhaust.** 5–10 representative files per area beat reading everything.
- **Dense cross-linking.** Every doc points to the relevant siblings by relative path (`[mapper-pattern.md](../patterns/mapper-pattern.md)`). This is what turns loose files into a navigable tree.
...
| Putting an invariant in `context.md` with no enforcing mechanism | "Enforced invariants" means enforced. Each line names the lint rule / hook / type / interceptor at its real path. A rule with no mechanism is an aspiration — move it to a "no enforcement" note or drop it. |
...
| Portuguese output | Content is English. Only identifiers/paths/lib names stay verbatim. |
...
| Inventing integrations/patterns/concerns to fill a directory | Evidence ≥2 occurrences for a pattern; real wiring for an integration; a `path:line` anchor for a concern. Omit otherwise. |
| Giving `concerns/` docs a `Golden rule`, or padding them with speculative risk | Concerns are evaluative: open at findings, every one anchored + severity-tagged. An honestly short `concerns/` beats an inflated one. |
| A concern written in prose (`Anchor:` without a line, severity as a sentence) | The concern contract is machine-parseable: `id`, `severity` enum, `anchor: path:line`. `sdd:plan` filters by path — prose can't be filtered. |
...
| Creating a top-level `cicd/`, `ci/` or `deploy/` dir for Docker/pipeline | CI/CD and container build live in `conventions/` (`dockerfile.md`, `<ci>-pipelines.md`). The tree is fixed at 6 dirs — never create a seventh. |
