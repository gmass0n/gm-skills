...
When the gate is fully green, update `state.md` (`next: —`, all REQs ✅) and report: which tasks landed, the matrix all-green, suite + coverage status, test-count delta, and any `SPEC_DEVIATION` markers (each a conscious, reviewed divergence). That report is the proof the user asked for.
